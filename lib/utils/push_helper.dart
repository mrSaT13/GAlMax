...
/// Звук уведомлений из настроек: имя raw-ресурса без расширения.
/// Пустая строка / 'system' = системный звук по умолчанию.
AndroidNotificationSound? notificationSound() {
  if (!AppSettings.notificationSoundEnabled.value) return null;
  final name = AppSettings.notificationSoundName.value;
  ...
  final androidPlatformChannelSpecifics = AndroidNotificationDetails(
    ...
    actions: event.type == EventTypes.RoomMember || !useNotificationActions
        ...
        : <AndroidNotificationAction>[
            AndroidNotificationAction(
              galmaxNotificationActions.reply.name,
              l10n.reply,
              inputs: [
            ...
            AndroidNotificationAction(
              galmaxNotificationActions.markAsRead.name,
              l10n.markAsRead,
              semanticAction: SemanticAction.markAsRead,
            ),
            AndroidNotificationAction(
              galmaxNotificationActions.mute.name,
              l10n.mute,
...
class galmaxPushPayload {
  final String? clientName, roomId, eventId;

  galmaxPushPayload(this.clientName, this.roomId, this.eventId);

  factory galmaxPushPayload.fromString(String payload) {
    final parts = payload.split('|');
    if (parts.length != 3) {
      return galmaxPushPayload(null, null, null);
    }
    return galmaxPushPayload(parts.first, parts[1], parts[2]);
  }
