...
Future<void> notificationTapBackground(
  ...
  // Сериализуем с FCM-фоном и синком: иначе гонка за sqlite.
  try {
    await CrossIsolateMutex.run('matrix_db', () async {
      await _handleTapInBackground(notificationResponse);
    });
  } on TimeoutException {
    Logs().w('[Tap] background tap skipped, db busy');
  }
}

Future<void> _handleTapInBackground(
  NotificationResponse notificationResponse,
) async {
  ...
  final payload = galmaxPushPayload.fromString(
...
Future<void> notificationTap(
  ...
  switch (notificationResponse.notificationResponseType) {
    case NotificationResponseType.selectedNotification:
      ...
      await client.roomsLoading?.timeout(const Duration(seconds: 30));
      await client.accountDataLoading?.timeout(const Duration(seconds: 30));
    ...
    case NotificationResponseType.selectedNotificationAction:
      final actionType = galmaxNotificationActions.values.singleWhereOrNull(
      ...
      await client.roomsLoading?.timeout(const Duration(seconds: 30));
      await client.accountDataLoading?.timeout(const Duration(seconds: 30));
      await client.userDeviceKeysLoading?.timeout(const Duration(seconds: 30));
      ...
      switch (actionType) {
        ...
        case galmaxNotificationActions.mute:
          await room.setPushRuleState(PushRuleState.mentionsOnly);
        case galmaxNotificationActions.open:
          router?.go(
            client.getRoomById(roomId)?.membership == Membership.invite
                ? '/rooms?client=${client.clientName}'
                : '/rooms/$roomId?client=${client.clientName}',
...
enum galmaxNotificationActions { markAsRead, reply, mute, open }
