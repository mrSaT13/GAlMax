...
extension InitWithRestoreExtension on Client {
  // Ключ бэкапа сессии. После переименования galmax -> GAlMax дефолт
  // applicationName сменился, поэтому старые установки хранят бэкап под
  // legacy-ключом 'galmax_session_backup_*'. Проверяем оба, чтобы не
  // потерять восстановление сессии при сбое базы.
  static String _backupKey(String appName, String clientName) =>
      '${appName}_session_backup_$clientName';

  static Future<String?> _readSessionBackup(
    FlutterSecureStorage? storage,
    String clientName,
  ) async {
    if (storage == null) return null;
    final currentKey = _backupKey(
      AppSettings.applicationName.value,
      clientName,
    );
    var value = await storage.read(key: currentKey);
    if (value != null) return value;
    // Legacy-ключи от прошлых названий.
    for (final legacyApp in ['galmax', 'FluffyChat', 'chat.fluffy']) {
      final legacyKey = _backupKey(legacyApp, clientName);
      if (legacyKey == currentKey) continue;
      value = await storage.read(key: legacyKey);
      if (value != null) {
        // Мигрируем на новый ключ, чтобы дальше читать одно место.
        await storage.write(key: currentKey, value: value);
  ...
  static Future<void> deleteSessionBackup(String clientName) async {
    ...
    await storage.delete(
      key: _backupKey(AppSettings.applicationName.value, clientName),
    );
    for (final legacyApp in ['galmax', 'FluffyChat', 'chat.fluffy']) {
      await storage.delete(key: _backupKey(legacyApp, clientName));
    }
  }

  Future<void> initWithRestore({void Function()? onMigration}) async {
    final storageKey = _backupKey(
      AppSettings.applicationName.value,
      clientName,
    );
    final storage = PlatformInfos.isMobile || PlatformInfos.isLinux
    ...
    try {
      ...
      final sessionBackupString = await _readSessionBackup(storage, clientName);
