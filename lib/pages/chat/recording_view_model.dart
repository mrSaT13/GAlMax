...
class RecordingViewModelState extends State<RecordingViewModel> {
  ...
  Future<void> startRecording(Room room) async {
    ...
    _room = room;
    // Собеседник видит «записывает голосовое…» (GalMax) / «печатает…» (чужие).
    GalmaxActivity.send(room, GalmaxActivity.recordingVoice);
  ...
  void _reset() {
    ...
    final room = _room;
    _room = null;
    if (room != null) GalmaxActivity.stopActivity(room);
  ...
  Future<void> stopAndSend(
    ...
    for (var i = 0; i < amplitudeTimeline.length; i += step) {
      final v = (amplitudeTimeline[i] / 100 * 1024).round().clamp(1, 1024);
      waveform.add(v);
    }
    // Пустой waveform (тап <100мс) — минимум, иначе плеер рисует пустоту.
    if (waveform.isEmpty) waveform.add(256);
    // Добиваем до waveCount повтором последнего — полоса стабильной длины.
    while (waveform.length < waveCount) {
      waveform.add(waveform.last);
    }
    ...
    // Не ждём аплоад: onVoiceMessageSend сам снимет активность в конце.
    // cancel() здесь нельзя — он шлёт stop и гасит «отправляет голосовое…».
    // Только сбрасываем UI записи, _room оставляем под аплоад.
    final sendFuture = onSend(path, duration.inMilliseconds, waveform, fileName!);
    if (mounted) {
      setState(() {
        WakelockPlus.disable();
        _recorderSubscription?.cancel();
