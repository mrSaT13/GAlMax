...
import '../../../widgets/galmax_app.dart';
  ...
  void dispose() {
    ...
    if (audioPlayer != null) {
      if (audioPlayer.playing && !audioPlayer.isAtEndPosition) {
        // Show MaterialBanner to control playback when scrolling away
                ...
                builder: (context, _) => GestureDetector(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Flexible(
                        child: Text(
                          '${audioPlayer.position.minuteSecondString} / ${audioPlayer.duration?.minuteSecondString} - ${widget.event.senderFromMemoryOrFallback.calcDisplayname()}',
                          maxLines: 1,
      ...
      // Stop proximity sensor when playback ends
  ...
  Future<void> _onButtonTap() async {
    ...
    if (currentPlayer != null && !currentPlayer.isAtEndPosition) {
      if (currentPlayer.playing) {
        currentPlayer.pause();
        // Stop proximity sensor when paused
        if (PlatformInfos.isMobile) {
          ProximityAudioHelper.instance.onPlaybackStopped();
        }
      } else {
        currentPlayer.play();
        // Start proximity sensor when resumed
        if (PlatformInfos.isMobile) {
          ProximityAudioHelper.instance.onPlaybackStarted();
        }
      }
      return;
    }
    ...
    // Start proximity sensor listening for earpiece switching
    if (PlatformInfos.isMobile) {
      ProximityAudioHelper.instance.onPlaybackStarted();
    }

    // Listen for playback completion to stop proximity sensor
                ...
                children: [
                  ConstrainedBox(
                    constraints: const BoxConstraints(
                      maxWidth: GalmaxThemes.columnWidth,
                    ),
                    child: Row(
                      mainAxisSize: .min,
                      children: <Widget>[
