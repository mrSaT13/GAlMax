...
class SettingsStyleController extends State<SettingsStyle> {
  ...
  /// Локальный фон приложения (только это устройство, без загрузки на сервер).
  Future<void> setLocalBackground() async {
    if (kIsWeb) return;
    final picked = await selectFiles(context, type: FileType.image);
    final pickedFile = picked.firstOrNull;
    if (pickedFile == null) return;
    ...
    await AppSettings.chatLocalBackgroundPath.setItem(path);
    if (mounted) setState(() {});
  }

  Future<void> deleteLocalBackground() async {
    await AppSettings.chatLocalBackgroundPath.setItem('');
    if (mounted) setState(() {});
  }
