...
import 'package:galmax/pages/chat_details/participant_list_item.dart';
import 'package:galmax/utils/auto_delete_helper.dart';
...
import 'package:galmax/widgets/chat_settings_popup_menu.dart';
...
class ChatDetailsView extends StatelessWidget {
              ...
              itemBuilder: (BuildContext context, int i) => i == 0
                  ? Column(
                      ...
                      children: <Widget>[
                        ...
                        Builder(
                          builder: (context) {
                            final currentRetention = AutoDeleteHelper.getCurrentRetention(room);
                            final currentLabel = AutoDeleteHelper.retentionOptions.entries
                                .where((e) => e.value == (currentRetention?.inMilliseconds ?? 0))
                                .map((e) => e.key)
                                .firstOrNull ?? 'Выкл';
                            return ListTile(
                              leading: CircleAvatar(
                              ...
                              subtitle: Text('Сейчас: $currentLabel'),
                              onTap: () => _showAutoDeleteDialog(context, room),
                              trailing: const Icon(Icons.chevron_right_outlined),
                            );
                          },
                        ),
  ...
  void _showAutoDeleteDialog(BuildContext context, Room room) {
    final currentRetention = AutoDeleteHelper.getCurrentRetention(room);
    final currentMs = currentRetention?.inMilliseconds ?? 0;
          ...
          child: Column(
            ...
            children: [
              ...
              const Padding(
                ...
                child: Text(
                  'Сообщения старше выбранного срока удалит сервер. '
                  'Работает, только если сервер поддерживает retention '
                  '(Synapse: с включённой политикой хранения).',
                  style: TextStyle(fontSize: 13, color: Colors.grey),
                ),
              ),
              const SizedBox(height: 8),
              ...AutoDeleteHelper.retentionOptions.entries.map(
                (entry) => RadioListTile<int>(
                  ...
                  onChanged: (value) async {
                    ...
                    try {
                      if (value == 0) {
                        await AutoDeleteHelper.removeRetention(room);
                      } else {
                        await AutoDeleteHelper.setRetention(
                          room,
                          Duration(milliseconds: value),
                        );
                      }
                      // Список выше перестроится сам: setRetention шлёт
                      // state-событие, onRoomState дёргает StreamBuilder.
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                              'Готово. Проверь, что сервер реально чистит старые сообщения.',
                            ),
                          ),
                        );
                      }
