...
import 'package:galmax/pages/chat_list/chat_list_view.dart';
...
import 'package:galmax/utils/show_update_snackbar.dart';
...
class ChatListController extends State<ChatList>
  ...
  bool Function(Room) getRoomFilterByActiveFilter(ActiveFilter activeFilter) {
    bool baseFilter(Room room) {
      switch (activeFilter) {
        ...
        case ActiveFilter.tag:
          return room.tags.keys.contains(activeTag);
      }
    }

    // Комната «Избранное» с собой скрыта из общего списка:
    // вход один — закреплённая плитка над списком. Иначе Избранное
    // показывается дважды (плитка + обычная строка комнаты).
    return (room) =>
        !FavoritesHelper.isSelfRoom(room) && baseFilter(room);
  }
  ...
  static const String _serverStoreNamespace = 'im.galmax.search.server';
  ...
  Future<void> _search() async {
    ...
    // Пустой запрос — серверный поиск не нужен, локальный фильтр в
    // chat_list_body сам покажет комнаты. Раньше слали пустой запрос на
    // сервер при каждом открытии поиска.
    if (searchQuery.isEmpty) {
      if (!isSearchMode || !mounted) return;
      setState(() {
        isSearching = false;
        this.roomSearchResult = null;
        this.userSearchResult = null;
      });
      return;
    }
    // Каждый серверный запрос — в своём try/catch и БЕЗ снекбара:
    // поиск best-effort, один упавший источник не должен занулять другой
    // и спамить «упс, что-то пошло не так» при каждом вводе (многие серверы
    // отключают user directory / public rooms).
    try {
      ...
      if (searchQuery.isValidMatrixId &&
        ...
        try {
          ...
          Logs().v('Room alias lookup failed (best-effort search)', e, s);
        }
      }
    } catch (e, s) {
      Logs().v('Public rooms search failed (best-effort)', e, s);
    }
    try {
      ...
      Logs().v('User directory search failed (best-effort)', e, s);
    }
  ...
  void initState() {
    ...
    // Если входящий (видео)звонок пришёл пока контекста не было, окно
    // лежит в очереди — показываем сразу как появился контекст.
    Matrix.of(context).voipPlugin?.flushPendingOverlays();
